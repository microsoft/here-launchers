...
namespace FullDemo
{
    public partial class DirectionsShowRouteToPage : PhoneApplicationPage
    {
        public DirectionsShowRouteToPage()
        {
            InitializeComponent();
        }
        ...
        private void Button_gridbut_Click(object sender, RoutedEventArgs e)
        {
            if (sender == LaunchButton)
            {
                try
                {
                    ...
                    routeTo.Destination = new GeoCoordinate(Double.Parse(LatitudeBox.Text), Double.Parse(LongittudeBox.Text));
